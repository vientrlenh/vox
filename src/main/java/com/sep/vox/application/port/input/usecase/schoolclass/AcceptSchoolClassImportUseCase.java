...
@Service
public class AcceptSchoolClassImportUseCase implements IUseCase<AcceptSchoolClassImportCommand, Void> {

    private static final Set<String> REQUIRED_FIELDS = Set.of("code", "name", "languageCode", "schoolGradeCode");
    private static final String SCHOOL_CLASS_TYPE = "SCHOOL_CLASS";
    ...
    @Override
    @Transactional
    public Void execute(AcceptSchoolClassImportCommand input) {
        ...
        var queued = importSessionRepository.markQueued(
                input.importSessionId(), SCHOOL_CLASS_TYPE, confirmedMappingJson, now, currentUserId);
        if (queued == 0) {
