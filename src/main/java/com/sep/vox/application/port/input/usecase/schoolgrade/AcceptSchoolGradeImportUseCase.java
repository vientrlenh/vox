...
@Service
public class AcceptSchoolGradeImportUseCase implements IUseCase<AcceptSchoolGradeImportCommand, Void> {

    private static final Set<String> REQUIRED_FIELDS = Set.of("schoolGradeLevelCode", "code", "name", "startDate", "endDate");
    private static final String SCHOOL_GRADE_TYPE = "SCHOOL_GRADE";
    ...
    public AcceptSchoolGradeImportUseCase(
    ...
    @Override
    @Transactional
    public Void execute(AcceptSchoolGradeImportCommand input) {
        ...
        var queued = importSessionRepository.markQueued(
                input.importSessionId(), SCHOOL_GRADE_TYPE, confirmedMappingJson, now, currentUserId);
        if (queued == 0) {
    ...
    private void validateCommand(AcceptSchoolGradeImportCommand input) {
    ...
    private void validateSession(ImportSession session, UUID schoolId, Instant now) {
        if (session.getType() != ImportType.SCHOOL_GRADE) {
            throw new IllegalArgumentException("Phiên import không phải là import năm học");
        }
