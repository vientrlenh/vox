...
@Service
public class AcceptSchoolGradeLevelImportUseCase implements IUseCase<AcceptSchoolGradeLevelImportCommand, Void> {

    private static final Set<String> REQUIRED_FIELDS = Set.of("code", "name", "order");
    private static final String SCHOOL_GRADE_LEVEL_TYPE = "SCHOOL_GRADE_LEVEL";
    ...
    public AcceptSchoolGradeLevelImportUseCase(
    ...
    public Void execute(AcceptSchoolGradeLevelImportCommand input) {
        ...
        var queued = importSessionRepository.markQueued(
                input.importSessionId(), SCHOOL_GRADE_LEVEL_TYPE, confirmedMappingJson, now, currentUserId);
        if (queued == 0) {
    ...
    private void validateCommand(AcceptSchoolGradeLevelImportCommand input) {
