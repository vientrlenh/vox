...
public class AcceptSchoolRoomImportUseCase implements IUseCase<AcceptSchoolRoomImportCommand, Void> {
    ...
    public AcceptSchoolRoomImportUseCase(
    ...
    public Void execute(AcceptSchoolRoomImportCommand input) {
        ...
        var queued = importSessionRepository.markQueued(
                input.importSessionId(), SCHOOL_ROOM_TYPE, confirmedMappingJson, now, currentUserId);
        if (queued == 0) {
    ...
    private void validateCommand(AcceptSchoolRoomImportCommand input) {
