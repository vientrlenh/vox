...
-- KHÔNG vá chk_import_sessions_type_valid ở đây nữa: V2 và V4 (nhánh default, đã merge) đều
-- đã thêm SCHOOL_ROOM/RUBRIC_RESULT_BAND với đúng cùng danh sách 12 giá trị. Khai lại lần thứ
-- ba là dựng sẵn cái bẫy "cùng một danh sách ở nhiều nơi" -- sửa một chỗ quên hai chỗ kia thì
-- ràng buộc cuối cùng chạy sẽ ghi đè, và không ai biết nơi nào mới là nguồn sự thật.

-- rubric_result_bands.score_min/score_max: V1 khai numeric(6,2) và không migration nào của
-- nhánh default đụng tới (kiểm ngày merge), nên vẫn cần nới ở đây.
