...
class AcceptSchoolGradeLevelImportUseCaseTests {
    ...
    private AcceptSchoolGradeLevelImportUseCase useCase;
    ...
    void setUp() {
        ...
        useCase = new AcceptSchoolGradeLevelImportUseCase(
    ...
    @Test
    void execute_should_mark_queued_with_grade_level_type() {
        when(importSessionRepository.findById(sessionId)).thenReturn(Optional.of(previewedSession()));
        when(importSessionRepository.markQueued(eq(sessionId), eq("SCHOOL_GRADE_LEVEL"), any(), any(), eq(userId)))
            .thenReturn(1);

        useCase.execute(new AcceptSchoolGradeLevelImportCommand(schoolId, sessionId, validMapping()));

        verify(importSessionRepository).markQueued(eq(sessionId), eq("SCHOOL_GRADE_LEVEL"), any(), any(), eq(userId));
    }
    ...
    void execute_should_throw_when_required_mapping_missing() {
        ...
        assertThatThrownBy(() -> useCase.execute(
                new AcceptSchoolGradeLevelImportCommand(schoolId, sessionId, Map.of("Mã khối", "code"))))
    ...
    void execute_should_throw_when_session_type_mismatch() {
        ...
        assertThatThrownBy(() -> useCase.execute(
                new AcceptSchoolGradeLevelImportCommand(schoolId, sessionId, validMapping())))
            .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void execute_should_throw_when_session_not_previewed() {
        when(importSessionRepository.findById(sessionId))
            .thenReturn(Optional.of(session(ImportType.SCHOOL_GRADE_LEVEL, ImportSessionStatus.COMPLETED)));
    ...
    private ImportSession previewedSession() {
        return session(ImportType.SCHOOL_GRADE_LEVEL, ImportSessionStatus.PREVIEWED);
    }

    private ImportSession session(ImportType type, ImportSessionStatus status) {
        return new ImportSession(
            sessionId, schoolId, type, "grade-levels.csv", "[]", "{}", null,
